...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is not None:
            ...
            for position in self.legalPositions:
                # true distance can be calculated by using manhattan distance from util
                # new belief = old belief * emission model probability
                allPossible[position] = self.beliefs[position] * \
                                        emissionModel[util.manhattanDistance(position, pacmanPosition)]
        # noisyDistance will be None when the ghost is captured.
        # therefore, put 1 for prison cell position.
        else:
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        "*** YOUR CODE HERE ***"
        allPossible = util.Counter()
        # for all old positions
        for oldPosition in self.legalPositions:
            # get probability distribution of new ghost positions given old position
            # iterate over these new positions and their respective probabilities
            for newPosition, probability in \
                    self.getPositionDistribution(self.setGhostPosition(gameState, oldPosition)).items():
                # update beliefs by adding old position belief * P(newPos|oldPos)
                allPossible[newPosition] += self.beliefs[oldPosition] * probability
